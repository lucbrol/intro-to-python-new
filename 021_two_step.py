...
def divide_by_two_and_add_one(num):
  return (num / 2) + 1 # Divide num by two and add one to the result
...
def multiply_by_forty_and_add_sixty(num):
  return (num * 40)  + 60 # Multiply num by forty, and then add sixty
...
def add_together_and_double(num_a, num_b):
  return (num_a + num_b) * 2# Add together num_a and num_b, then double the result
